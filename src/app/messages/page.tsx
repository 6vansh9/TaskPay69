    ...
    <div className="flex flex-col h-screen bg-card">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <aside className={cn(
          ...
          // On mobile, hide sidebar when a conv is active on desktop
            ...
            {!loading && filtered.map(conv => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'w-full flex items-center gap-3 px-4 py-3.5 hover:bg-card transition-colors text-left border-b border-border last:border-0',
                    isActive && 'bg-[#14a800/10] border-l-[3px] border-l-[#14a800]',
                    !isActive && hasUnread && 'bg-card'
                  )}
                >
                  {/* Avatar */}
                  <div className="relative flex-shrink-0">
                    {other?.avatar_url ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-[#14a800/20] flex items-center justify-center text-base font-bold text-[#14a800]">
                        {(other?.company_name ?? other?.full_name ?? '?')[0].toUpperCase()}
                      </div>
                    )}
                    {hasUnread && (
                      <span className="absolute -top-0.5 -right-0.5 w-5 h-5 rounded-full bg-[#14a800] text-foreground text-[9px] font-bold flex items-center justify-center border-2 border-white">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1 mb-0.5">
                      <span className={cn('text-sm truncate', hasUnread ? 'font-bold text-foreground' : 'font-semibold text-foreground')}>
        ...
        <main className="flex-1 hidden lg:flex flex-col min-w-0 bg-card">
          {activeConvId && activeOther && userId ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              <div className="w-24 h-24 rounded-full bg-[#14a800/20] flex items-center justify-center mb-5">

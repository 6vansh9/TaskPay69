...
interface JobInfo { title: string; budget_type: string; budget_min: number | null; budget_max: number | null; category: string | null }
...
function ApplyContent() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const [{ data: job }, { data: profile }] = await Promise.all([
        supabase.from('jobs').select('title, budget_type, budget_min, budget_max, category, status').eq('id', jobId).single(),
        supabase.from('profiles').select('role, connects_balance').eq('id', user.id).single(),
      ])

      if (!job || job.status !== 'open') { toast.error('This job is no longer available'); router.push('/jobs'); return }
      if (profile?.role !== 'freelancer') { toast.error('Only freelancers can submit proposals'); router.push(`/jobs/${jobId}`); return }

      // Check existing proposal
      const { data: existing } = await supabase.from('proposals').select('id').eq('job_id', jobId).eq('freelancer_id', user.id).single()
      if (existing) { toast.error('You already applied to this job'); router.push(`/jobs/${jobId}`); return }

      setJobInfo(job)
      setConnectsBalance(profile?.connects_balance ?? 0)
      setLoading(false)
    })
  }, [jobId, router])
  ...
  if (loading) return (
    <div className="flex flex-col min-h-screen bg-[#f7f7f7]"><Header />
    ...
    <div className="flex flex-col min-h-screen bg-[#f7f7f7]">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-6">
        ...
        <nav className="text-sm text-[#6b6b6b] mb-4 flex items-center gap-1">
          ...
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-[#1d1d1d]">Apply</span>
        </nav>

        {/* Job summary */}
        <div className="card p-4 mb-4 flex items-center justify-between flex-wrap gap-2">
          <div>
            <h2 className="font-semibold text-[#1d1d1d]">{jobInfo?.title}</h2>
          ...
          <div className="text-right">
            ...
            <div className="text-xs text-[#6b6b6b] capitalize">{jobInfo?.budget_type}</div>
          </div>
        </div>
        ...
        {!hasEnoughConnects && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
            ...
            <div>
              <p className="text-sm font-medium text-red-700">Not enough connects</p>
              <p className="text-xs text-red-600">This job costs {cost} connects. You have {connectsBalance}.</p>
            </div>
          </div>
        )}

        {/* Connects cost banner */}
        {hasEnoughConnects && (
          <div className="flex items-center gap-3 bg-[#f0fdf4] border border-[#14a800]/30 rounded-xl p-4 mb-4">
            <Zap className="w-4 h-4 text-[#14a800] flex-shrink-0" />
            <p className="text-sm text-[#0a6300]">
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-5">
          <h1 className="text-xl font-bold text-[#1d1d1d]">Submit your proposal</h1>
          ...
          <div className="field">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6b6b6b] font-medium">₹</span>
              <input
          ...
          {/* Cover letter */}
          <div className="field">
            <label className="label">Cover letter <span className="text-red-500">*</span></label>
            <textarea
            ...
            <div className="flex justify-between">
              ...
              <span className={cn('text-xs ml-auto', form.cover_letter.length > 1400 ? 'text-amber-500' : form.cover_letter.length < 100 ? 'text-[#6b6b6b]' : 'text-[#14a800]')}>
                {form.cover_letter.length}/1500
              </span>
            </div>
            {form.cover_letter.length >= 100 && form.cover_letter.length < 100 && (
              <div className="progress-track mt-1"><div className="progress-fill" style={{ width: `${(form.cover_letter.length / 100) * 100}%` }} /></div>
            )}
          </div>

          {/* Optional attachment */}
          <div className="field">
            <label className="label">Portfolio / attachment link <span className="text-[#aaa] font-normal">(optional)</span></label>
            <input
...
export default function ApplyPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f7f7f7]"><Header /><main className="max-w-2xl mx-auto px-4 py-8 space-y-4">{Array.from({length:3}).map((_,i)=><div key={i} className="card p-5 h-32 skeleton"/>)}</main></div>}>
      <ApplyContent />
    </Suspense>
  )
}

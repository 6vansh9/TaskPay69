...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from('profiles')
    .select(
      'id, full_name, avatar_url, title, bio, hourly_rate, skills, skills_with_level, location, rating, review_count, jobs_completed, phone_verified, edu_verified, is_available, created_at',
      { count: 'exact' }
    )
    .eq('role', 'freelancer')
  ...
  if (sort === 'most_completed') {
    query = query.order('jobs_completed', { ascending: false }).order('rating', { ascending: false })
  } else if (sort === 'newest') {

...
export default function FloatingChat({ userId, userRole }: Props) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {/* Chat panel */}
      {open && !minimized && (
        <div className="w-80 bg-white rounded-2xl shadow-2xl border border-[#e0e0e0] overflow-hidden flex flex-col"
          style={{ height: activeConvId ? '480px' : '320px' }}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#14a800] text-white flex-shrink-0">
          ...
          {activeConvId && otherParty ? (
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.length === 0 ? (
                <div className="text-center text-sm text-[#6b6b6b] py-10 px-4">
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#f7f7f7] transition-colors border-b border-[#f0f0f0] last:border-0 text-left"
                  >
                    {other?.avatar_url ? (
                      <img src={other.avatar_url} alt="" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
                    ) : (
                      <div className="w-9 h-9 rounded-full bg-[#e2f0d9] flex items-center justify-center text-sm font-bold text-[#14a800] flex-shrink-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-[#6b6b6b] truncate">{conv.job?.title ?? 'Contract chat'}</div>
                    </div>
                  </button>
                )
              })}
      ...
      {open && minimized && (
        <div
          ...
          className="bg-[#14a800] text-white px-4 py-2 rounded-xl shadow-lg cursor-pointer flex items-center gap-2 text-sm font-medium hover:bg-[#108200] transition-colors"
        >
          <MessageSquare className="w-4 h-4" />
          Messages
      ...
      {!open && (
        <button
          ...
          className={cn(
            'w-14 h-14 rounded-full bg-[#14a800] hover:bg-[#108200] text-white shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95',
          )}

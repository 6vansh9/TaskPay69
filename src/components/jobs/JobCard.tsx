...
export default function JobCard({ job, freelancerSkills = [], saved: savedInit = false }: Props) {
  ...
  useEffect(() => {
    ...
    // Unique name prevents Strict Mode's double-mount from reusing a still-subscribed channel
    const name = `job-bid-count-${job.id}-${Math.random().toString(36).slice(2)}`
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        setBidCount(prev => updated.proposals_count ?? updated.bid_count ?? prev)
      })
      .subscribe()
  ...
  return (
    <div className="card card-hover p-5 flex flex-col gap-3 transition-all cursor-pointer group">
      ...
      <Link href={`/jobs/${job.id}`}>
        <h3 className="font-semibold text-foreground text-base group-hover:text-[#14a800] transition-colors line-clamp-2">
          {job.title}
        </h3>
      </Link>
